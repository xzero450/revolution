...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker)
{
	...
	if (targ->client->pers.teamState.lasthurtcarrier &&
		...
		G_LogPrintf("Hurt_Carrier_Defend: %i\n", attacker - g_entities);
...
int Team_TouchOurFlag(gentity_t *ent, gentity_t *other, int team)
{
	...
	int			enemy_flag, team_flag, enemy_team;
	int			counts[TEAM_NUM_TEAMS];
	int			captureTime, captureTimeSec, captureTimeMin;

	counts[team] = TeamCount(-1, team);
...
//		else if(cl->sess.sessionTeam == TEAM_RED)
		...
		{
			//flag isn't at home
			if(!cl->ps.powerups[team_flag] && !cl->ps.powerups[enemy_flag])
			{
				//not holding any flag yet
				if(((g_returnFlag.integer == 2) && (counts[team] >= 2)) || (g_returnFlag.integer == 1))
				{
					//manually return the flag
					if(team == TEAM_RED)
						cl->ps.powerups[PW_REDFLAG] = INT_MAX;	//flags never expire
					else
						cl->ps.powerups[PW_BLUEFLAG] = INT_MAX;	//flags never expire

					//send global sound when your team picks your dropped flag
					if(g_IU_ctf_sounds.integer)
					{
						gentity_t *te;
						te = G_TempEntity( ent->s.pos.trBase, EV_GLOBAL_TEAM_SOUND );
						te->s.eventParm = GTS_PICKED_UP_DROP_TEAM_FLAG;
						te->r.svFlags |= SVF_BROADCAST;
					}

					trap_SendServerCommand(-1, va("print \"%s" S_COLOR_WHITE " is returning the %s flag!\n", cl->pers.netname, TeamName(team)));
					cl->pers.stats_flags[1]++;					//stats team touches
					G_LogPrintf("Flag_Pickup: %i\n", other - g_entities);

					//for monkk's flag sacrifice
					if(g_flagSacrifice.integer != 0)
						other->flagSacData.sacrificeCount = 0;

					Team_SetFlagStatus( team, FLAG_RETURNING );
//					G_Printf("Pick up flag for manual returning\n");
				...
				else
				{
					...
					cl->pers.stats_flags[1]++;					//stats team touches
					cl->pers.stats_flags[3]++;					//stats return

					PrintMsg(NULL, "%s" S_COLOR_WHITE " returned the %s flag!\n", cl->pers.netname, TeamName(team));
					cl->ps.powerups[team_flag] = 0;
					...
					Team_ReturnFlagSound(Team_ResetFlag(team), team);
					G_LogPrintf("Flag_Return: %i\n", other - g_entities);
//					G_Printf("return our flag(Team_ResetFlag)\n");

					return 0;
				}
			}
			else if((g_returnFlag.integer == 2) && (counts[team] < 2) || (g_returnFlag.integer == 0))
			{
				//teleport the flag back home
				cl->pers.stats_flags[1]++;					//stats team touches
				cl->pers.stats_flags[3]++;					//stats return
...
//				G_Printf("return our flag(Team_ResetFlag)\n");
		...
		else if(cl->ps.powerups[team_flag])
		{
			//the flag was returned to home manually
			cl->pers.stats_flags[1]++;					//stats team touches
			cl->pers.stats_flags[3]++;					//stats return
...
//			G_Printf("return our flag(Team_ResetFlag)\n");
	...
	//flag is home, if the player has the enemy flag, he's capped
	if(!cl->ps.powerups[enemy_flag])
	{
//		G_Printf("no enemy flag\n");
//		G_Printf("%i\n", cl->ps.clientNum);
//		G_Printf("Team: %i Enemy: %i\n", cl->ps.powerups[team_flag], cl->ps.powerups[enemy_flag]);
	...
	else
	{
		...
		captureTime = (level.time - level.flag_pickupTime[enemy_team]);
		captureTimeSec = captureTime / 1000;
		captureTime -= (captureTimeSec * 1000);
		if(captureTimeSec > 60)
		{
			captureTimeMin = captureTimeSec / 60;
			captureTimeSec -= captureTimeMin * 60;
			PrintMsg(NULL, "" S_COLOR_CYAN "The %s team captured the flag in %i:%i.%i\n", TeamName(team), captureTimeMin, captureTimeSec, captureTime);
		}
		else
			PrintMsg(NULL, "" S_COLOR_CYAN "The %s team captured the flag in %i.%i\n", TeamName(team), captureTimeSec, captureTime);

#ifdef MISSIONPACK
	}
#endif

	cl->pers.stats_flags[4] += 1;								//stats captures
...
*/
	...
	G_LogPrintf("Flag_Capture: %i\n", other - g_entities);
	G_LogPrintf("TeamScore: 1 %i RED\n", level.teamScores[TEAM_RED]);
	G_LogPrintf("TeamScore: 2 %i BLUE\n", level.teamScores[TEAM_BLUE]);
	...
	//let's do the player loop, hand out the bonuses
	for(i = 0; i < g_maxclients.integer; i++)
	{
		player = &g_entities[i];
		if(!player->inuse)
		...
		else if(player->client->sess.sessionTeam == cl->sess.sessionTeam)
		{
			...
			if((player->client->pers.teamState.lastreturnedflag + CTF_RETURN_FLAG_ASSIST_TIMEOUT) > level.time)
			{
				...
				G_LogPrintf("Flag_Assist_Return: %i\n", other - g_entities);
...
//	G_Printf("Should've captured by now\n");
...
//	} else if ( cl->sess.sessionTeam == TEAM_RED ) {
	...
	trap_SendServerCommand( -1, va("print \"DBG:TouchOurFlag: clientNum: %i -- team: %i\n\"", other->client->ps.clientNum, team));
	trap_SendServerCommand( -1, va("print \"DBG:TouchOurFlag: enemy: %i -- ourFlag: %i -- theirFlag: %i\n\"", enemy_team, team_flag, enemy_flag));

	if ((ent->flags & FL_DROPPED_ITEM) && ((g_returnFlag.integer == 2 && counts[team] >= 2 && !other->client->ps.powerups[team_flag]) || (g_returnFlag.integer == 1 && !other->client->ps.powerups[team_flag]))) {
		//Return the flag manually.
		trap_SendServerCommand( -1, va("print \"DBG:Return manually\n\""));
		if (team == TEAM_RED) {
			cl->ps.powerups[PW_REDFLAG] = INT_MAX; // flags never expire
		} else {
			cl->ps.powerups[PW_BLUEFLAG] = INT_MAX; // flags never expire
		...
		//G_Printf("Pick up flag for manual returning\n");
		return -1;
	} else if ( ent->flags & FL_DROPPED_ITEM || cl->ps.powerups[team_flag] ) {
		trap_SendServerCommand( -1, va("print \"DBG: Auto return\n\""));
		cl->pers.stats_flags[1]++;//Stats team touches
		cl->pers.stats_flags[3]++;//Stats return
...
int Team_TouchEnemyFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	BaseFlag = (ent->s.modelindex2 == 0);
	if (!(ent->flags & FL_DROPPED_ITEM)) {
		level.flag_pickupTime[team] = level.time;
	}
	...
	if( g_gametype.integer == GT_1FCTF ) {
		...
		//cl->ps.powerups[enemy_flag] = INT_MAX;
	...
	if( g_flagSacrifice.integer != 0 ) {
		other->flagSacData.sacrificeCount = 0;//monkk flag sacrifice
		//set the player's LKG just in case they manage to grab the flag and fly off the map without touching the ground
		//setting it to the origin of the flag item lest it is in a hard to reach place that encourges further sacrifices
		//during retrieval
		VectorCopy(ent->r.currentOrigin, level.playerLastKnownGood[other - g_entities][0]);
		VectorCopy(ent->r.currentOrigin, level.playerLastKnownGood[other - g_entities][1]);
	}
//	Team_TakeFlagSound( ent, team );
	//send  global sound when your team picks up your dropped flag
	//Team_TakeFlagSound( ent, team );
	cl->pers.stats_flags[2]++;//Stats enemy touches

	AddScore(other, ent->r.currentOrigin, CTF_FLAG_BONUS);
	cl->pers.teamState.flagsince = level.time;
	Team_TakeFlagSound( ent, team );
	G_LogPrintf("Flag_Pickup: %i\n", other - g_entities);
	//This either fixes the disappearing flag bug,  or breaks manual flag return
	//if we're not lucky, it does neither.
	if (BaseFlag && !level.baseFlagSet[team]) {
	//	G_Printf("%i\n", cl->ps.clientNum);
	//	level.flag_pickupTime[team] = level.time;
		//G_Printf("BaseFlag\n");
...
int Pickup_Team( gentity_t *ent, gentity_t *other ) {
	...
	// GT_CTF
	if( team == cl->sess.sessionTeam) {
	//	G_Printf("Team_TouchOurFlag\n");
		return Team_TouchOurFlag( ent, other, team );
	}
	//G_Printf("Team_TouchEnemyFlag\n");
...
gentity_t *Team_GetLocationByVec(vec3_t origin)
{
	gentity_t		*eloc, *best;
	float			bestlen, len;
...
//monkk
qboolean Team_GetLocationMsgByVec(vec3_t origin, char *loc, int loclen)
{
	gentity_t *best;

	best = Team_GetLocationByVec( origin );
