...
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname) {
	...
	Info_SetValueForKey( userinfo, key, s );*/
	key = "color1";
	color1 = Info_ValueForKey( botinfo, key );
	if ( !*color1 ) {
		color1 = "1";
	}
	//Info_SetValueForKey( userinfo, key, s );
	...
	key = "color2";
	color2 = Info_ValueForKey( botinfo, key );
	if ( !*color2 ) {
		color2 = "6";
	}
	//key = "color1";
	//Info_SetValueForKey( userinfo, key, s );
	//PM Skin / Colors for bots.
	Com_sprintf(colors, sizeof(colors), "%s777%s", color1, color2);
	Info_SetValueForKey( userinfo, "colors", colors );
