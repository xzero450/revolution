...
static void UI_SPLevelMenu_ResetDraw( void ) {
	UI_DrawProportionalString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 0, "WARNING: This resets all of the", UI_CENTER|UI_SMALLFONT, 0.75f, color_yellow );
	UI_DrawProportionalString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 1, "single player game variables.", UI_CENTER|UI_SMALLFONT, 0.75f, color_yellow );
	UI_DrawProportionalString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 2, "Do this only if you want to", UI_CENTER|UI_SMALLFONT, 0.75f, color_yellow );
	UI_DrawProportionalString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 3, "start over from the beginning.", UI_CENTER|UI_SMALLFONT, 0.75f, color_yellow );
}
...
//	int				fraglimit;
	...
	Q_CleanStr( string );
	UI_DrawProportionalString( 320, PLAYER_Y, string, UI_CENTER|UI_SMALLFONT, 0.75f, color_orange );
	...
	UI_DrawProportionalString( 18, 38, va( "Tier %i", selectedArenaSet + 1 ), UI_LEFT|UI_SMALLFONT, 0.75f, color_orange );
	...
	if ( selectedArenaSet > currentSet ) {
		UI_DrawProportionalString( 320, 216, "ACCESS DENIED", UI_CENTER|UI_BIGFONT, 1.0f, color_main );
		return;
	}
	...
	UI_DrawProportionalString( 320, y, string, UI_CENTER|UI_SMALLFONT, 0.75f, color_orange );
...
//	UI_DrawString( 18, 212, va("Frags %i", fraglimit) , UI_LEFT|UI_SMALLFONT, color_orange );
	...
	for( n = 0; n < levelMenuInfo.numBots; n++ ) {
		...
		if( levelMenuInfo.botPics[n] ) {
			...
			UI_DrawProportionalString( x+22, y+18, "?", UI_BIGFONT, 1.0f, color_orange );
		}
...
static void UI_SPLevelMenu_Init( void ) {
	...
	levelMenuInfo.item_banner.color					= color_main;
