...
=================
*/
void UI_DrawProportionalString( int x, int y, const char* str, int style, float sizeScale, vec4_t color ) {
	vec4_t	drawcolor;
	int		width;
	//float	sizeScale;
	...
	while (1) {
		...
		*s3 = c_bcp;
		if (width > xmax) {
			...
			*s2 = '\0';
			UI_DrawProportionalString(x, y, s1, style, sizeScale, color);
			y += ystep;
			if (c_bcp == '\0')
      {
        ...
        if (*s2 != '\0') // if we are printing an overflowing line we have s2 == s3
          UI_DrawProportionalString(x, y, s2, style, sizeScale, color);
				break; 
      }
			s2++;
			s1 = s2;
...
void UI_DrawString( int x, int y, const char* str, int style, vec4_t color )
{
	...
	if (style & UI_SMALLFONT || style & UI_EXTRASMALLFONT) {
