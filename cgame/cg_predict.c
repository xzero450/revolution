...
/*
====================
CG_ClipMoveToEntities_Bitmask

====================
*/
static void CG_ClipMoveToEntities_Bitmask ( const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
							int skipNumber, int mask, trace_t *tr ) {
	int			i, x, zd, zu;
	trace_t		trace;
	entityState_t	*ent;
	...
	for ( i = 0 ; i < cg_numSolidEntities ; i++ ) {
		...
		if (trace.allsolid || trace.fraction < tr->fraction) {
			if (ent->number >= 0 && ent->number < 25)
			{
				clients |= 1 << ent->number;
				tr->entityNum = clients;
			}
			//*tr = trace;
		} else if (trace.startsolid) {
			tr->startsolid = qtrue;
...
================
*/
void	CG_Trace_Bitmask( trace_t *result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, 
					 int skipNumber, int mask ) {
	trace_t	t;
	...
	trap_CM_BoxTrace ( &t, start, end, lmins, lmaxs, 0, mask);
	//t.entityNum = t.fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
	// check all other solid models
	t.entityNum = 0;
	CG_ClipMoveToEntities_Bitmask (start, lmins, lmaxs, end, skipNumber, mask, &t);
...
static void CG_TouchItem( centity_t *cent ) {
	...
	//if ( (!cg_predictItems.integer) || (g_gamemode.integer != 0 || g_gamemode.integer != 4) ) {
	...
	if ( cg.time < cg.naftInterval && cgs.gametype == GT_CTF && cg.lastHeldFlag == item->giTag ) {
		return;
	}
...
//unlagged - optimized prediction
	...
	if ( sv_lock_pmove.integer == 1 ) {
		...
		cg_pmove.pmove_fixed = cg_pmove_fixed.integer;
	}
...
//unlagged - optimized prediction
	...
	for ( cmdNum = current - CMD_BACKUP + 1 ; cmdNum <= current ; cmdNum++ ) {
		...
		if ( cg_pmove.pmove_fixed ) {
			PM_UpdateViewAngles( cg_pmove.ps, &cg_pmove.cmd, 0 );
		}
		...
		cg_pmove.cmd.weapon = cg.weaponSelect;
...
//unlagged - optimized prediction
	...
	// This is for cg_aimType 2 style aim, but cg_aimType 3
	// falls back to 2 if it fails due to lag, so still need to send for 3

	if ( (cg_aimType.integer == 2 || cg_aimType.integer == 3) && g_delagHitscan.integer >= 2) {
		clientsHit = CheckFireHitAimType2(); //(pm, &pml);
		if (clientsHit) {
			cg.clientsHit = 1 << (clientsHit - 1);
		}
		...
		//cg.weaponSelect |= clientsHit;


		// let the client system know what our weapon and zoom settings are
		//trap_SetUserCmdValue( cg.weaponSelect, cg.zoomSensitivity );
		trap_SetUserCmdValue( clientsHit, cg.zoomSensitivity );

	} else {
		trap_SetUserCmdValue( cg.weaponSelect, cg.zoomSensitivity );
	}

	/*if ( cgs.dbgrws && cg.weaponSelectLastFrame != cg.weaponSelect ) {
		CG_Printf("DBGRWS:Should apply a switch this frame. - old:%i new:%i\n", cg.weaponSelectLastFrame, cg.weaponSelect);
	}*/

	if (cg.weaponSelectLastFrame != cg.weaponSelect && cg_aimType.integer >= 2 && g_delagHitscan.integer >= 2) {
		trap_SendClientCommand(va("rws %i\n", cg.weaponSelect));
		cg.weaponSelectLastFrame = cg.weaponSelect;
	}
...
int G_TimeShiftClientNum( int clientNum, int time, vec3_t currentOrigin, vec3_t mins, vec3_t maxs ) 
{
	return 0;
}

void G_UnTimeShiftClientNum( int clientNum, vec3_t currentOrigin, vec3_t mins, vec3_t maxs ) 
{

}


void G_EraseSaved( int clientNum ) 
{
