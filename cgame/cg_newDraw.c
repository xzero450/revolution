  ...
  if (shader) {
    ...
    if (item) {
	    ...
	    if( cgs.flags[TEAM_BLUE-1] >= 0 && cgs.flags[TEAM_BLUE-1] <= 3 ) {
		    CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.flagShaders[cgs.flags[TEAM_BLUE-1]] );
			} else {
  ...
  if (shader) {
    ...
    if (item) {
	    ...
	    if( cgs.flags[TEAM_RED-1] >= 0 && cgs.flags[TEAM_RED-1] <= 3) {
		    CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.flagShaders[cgs.flags[TEAM_RED-1]] );
			} else {
		    CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.flagShaders[0] );
			}
		  trap_R_SetColor(NULL);
...
qboolean CG_OtherTeamHasFlag() {
	if (cgs.gametype == GT_CTF || cgs.gametype == GT_1FCTF) {
		...
		if (cgs.gametype == GT_1FCTF) {
			...
			if (team == TEAM_RED && cgs.flags[TEAM_RED-1] == FLAG_TAKEN) {
				return qtrue;
			} else if (team == TEAM_BLUE && cgs.flags[TEAM_BLUE-1] == FLAG_TAKEN) {
				return qtrue;
			} else {
				return qfalse;
			}
...
qboolean CG_YourTeamHasFlag() {
	if (cgs.gametype == GT_CTF || cgs.gametype == GT_1FCTF) {
		...
		if (cgs.gametype == GT_1FCTF) {
			...
			if (team == TEAM_RED && cgs.flags[TEAM_BLUE-1] == FLAG_TAKEN) {
				return qtrue;
			} else if (team == TEAM_BLUE && cgs.flags[TEAM_RED-1] == FLAG_TAKEN) {
				return qtrue;
			} else {
				return qfalse;
			}
...
qboolean CG_OwnerDrawVisible(int flags) {
	...
	if (flags & (CG_SHOW_BLUE_TEAM_HAS_REDFLAG | CG_SHOW_RED_TEAM_HAS_BLUEFLAG)) {
		if (flags & CG_SHOW_BLUE_TEAM_HAS_REDFLAG && (cgs.redflag == FLAG_TAKEN || cgs.flags[TEAM_RED-1] == FLAG_TAKEN_RED)) {
			return qtrue;
		} else if (flags & CG_SHOW_RED_TEAM_HAS_BLUEFLAG && (cgs.flags[TEAM_BLUE-1] == FLAG_TAKEN || cgs.flagStatus == FLAG_TAKEN_BLUE)) {
			return qtrue;
		}
		return qfalse;
	}
