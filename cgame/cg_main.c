...
//PM Skin colors change variables
...
int vmMain( int command, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9, int arg10, int arg11  ) {
	...
	case CG_MOUSE_EVENT:
#ifdef MISSIONPACK
		cg.cursorx = cgs.cursorX;
		cg.cursory = cgs.cursorY;
#endif
...
//Superhud - Being
hud_t				hud[maxHudCommands + 64];//max number of commands maxHudCommands+64 == 66+64 == 130
int					hud_count = 0;
chat_t				chat[MAX_CHATS];
//Superhud - End
autoFile_t			autoFile;
...
vmCvar_t	cg_fullWeaponCycle;
vmCvar_t	rate;//Lucron: players command
vmCvar_t	com_maxfps;//Lucron: players command
vmCvar_t	cl_packetdup;//Lucron: players command
vmCvar_t	snaps;//Lucron: players command
vmCvar_t	cl_maxpackets;//Lucron: players command
vmCvar_t	cl_guid;
vmCvar_t	sex;
vmCvar_t	r_vertexLight;//PM Skins -- WE NEED THIS!
vmCvar_t	language;
...
vmCvar_t	colors;//PM Skin color for localclient
vmCvar_t	color1;//Replaced by colors[0]
vmCvar_t	color2;//Replaced by colors[4]
vmCvar_t	s_announcer;
vmCvar_t	cg_fixedTeleporterAngles;
vmCvar_t	cg_railHum;
vmCvar_t	cg_adjustTeamColors;
//Superhud - Begin
vmCvar_t	ch_file;
//Superhud - End
vmCvar_t	s_ambient;//enable ambiant sounds
vmCvar_t	cg_autoScreenshot;
vmCvar_t	cg_autoRecord;
vmCvar_t	cg_crosshairPulse;
vmCvar_t	cg_oldScoreboard;
vmCvar_t	ch_shadowOffset;
vmCvar_t	cg_redName;
...
static cvarTable_t cvarTable[] = { // bk001129
	{ &sex, "sex", "", CVAR_ARCHIVE | CVAR_USERINFO },
	{ &colors, "colors", "17776", CVAR_ARCHIVE | CVAR_USERINFO },//PM Skin color for localclient
	{ &color1, "color1", "4", CVAR_ROM },//Replaced by colors[0]
	{ &color2, "color2", "5", CVAR_ROM },//Replaced by colors[4]
	{ &com_maxfps, "com_maxfps", "85", CVAR_ARCHIVE | CVAR_USERINFO },
	{ &rate, "rate", "3000", CVAR_ARCHIVE | CVAR_USERINFO },
	{ &cg_aimType, "cg_aimType", "2", CVAR_ARCHIVE | CVAR_USERINFO  },
	{ &cg_aimNudge, "cg_aimNudge", "0", CVAR_USERINFO | CVAR_CHEAT  },
	{ &snaps, "snaps", "20", CVAR_ARCHIVE | CVAR_USERINFO },
	{ &cl_maxpackets, "cl_maxpackets", "30", CVAR_ARCHIVE | CVAR_USERINFO },
	{ &cl_guid, "cl_guid", "", CVAR_USERINFO },
	{ &g_allowReady, "g_allowReady", "1", CVAR_SERVERINFO },
	...
	{ &cg_crosshairHealth, "cg_crosshairHealth", "0", CVAR_ARCHIVE },//Was 1
...
//freeze
...
#endif
	{ &cg_drawTeamOverlay, "cg_drawTeamOverlay", "0", CVAR_ARCHIVE | CVAR_USERINFO },
	...
	{ &sv_lock_pmove, "sv_lock_pmove", "0", CVAR_SYSTEMINFO},
	//{ &sv_max_timeNudge, "sv_max_timeNudge", "0", CVAR_SYSTEMINFO | CVAR_SERVERINFO},
...
//unlagged - client options
	...
	{ &cg_fastRespawn_override, "cg_fastRespawn_override", "0", CVAR_ARCHIVE | CVAR_USERINFO },


		/*Weapon reload cycles*/
	{ &weap_enableHook, "weap_enableHook", "0", 0, },//monkk
	{ &weap_gauntletCycle, "weap_gauntletCycle", "400", 0 },
	{ &weap_machinegunCycle, "weap_machinegunCycle", "100", 0 },
	{ &weap_shotgunCycle, "weap_shotgunCycle", "1000", 0 },
	{ &weap_grenadelauncherCycle, "weap_grenadelauncherCycle", "800", 0 },
	{ &weap_lightninggunCycle, "weap_lightninggunCycle", "50", 0 },
	{ &weap_rocketlauncherCycle, "weap_rocketlauncherCycle", "800", 0 },
	{ &weap_railgunCycle, "weap_railgunCycle", "1500", 0 },
	{ &weap_plasmagunCycle, "weap_plasmagunCycle", "100", 0 },
	{ &weap_bfgCycle, "weap_bfgCycle", "200", 0 },
	{ &g_DynamicGauntletCycle, "g_DynamicGauntletCycle", "0", CVAR_SYSTEMINFO },
	{ &g_interference, "g_interference", "1", CVAR_SYSTEMINFO },

	//#################################################
	{ &g_gamemode, "g_gamemode", "0", CVAR_SERVERINFO | CVAR_SYSTEMINFO },
	{ &g_delagHitscan, "g_delagHitscan", "3", CVAR_SERVERINFO | CVAR_SYSTEMINFO },
	{ &g_pro_mode, "g_pro_mode", "0", CVAR_SERVERINFO },
	...
	{ &cg_TeamColors, "cg_TeamColors", "77777", CVAR_ARCHIVE },
	...
	{ &g_fastSwitch, "g_fastSwitch", "0", CVAR_SYSTEMINFO | CVAR_SERVERINFO },
	{ &r_vertexLight, "r_vertexLight", "0", CVAR_ARCHIVE | CVAR_LATCH },//PM Skins -- WE NEED THIS!
	{ &cl_packetdup, "cl_packetdup", "1", CVAR_ARCHIVE | CVAR_USERINFO }, //Lucron players command
	{ &s_announcer, "s_announcer", "feedback", CVAR_ARCHIVE },
	{ &cg_fixedTeleporterAngles, "cg_fixedTeleporterAngles", "0", CVAR_ARCHIVE | CVAR_USERINFO },
	{ &cg_railHum, "cg_railHum", "1", CVAR_ARCHIVE },
	{ &cg_adjustTeamColors, "cg_adjustTeamColors", "0", CVAR_ARCHIVE },
	{ &language, "language", "en", CVAR_ARCHIVE },
	{ &cg_autoScreenshot, "cg_autoScreenshot", "0", CVAR_ARCHIVE },
	{ &cg_autoRecord, "cg_autoRecord", "0", CVAR_ARCHIVE },
	{ &cg_motdDisplayTime, "cg_motdDisplayTime", "10", CVAR_ARCHIVE },
	//Superhud - Begin
	{ &ch_file, "ch_file", "", CVAR_ARCHIVE }, //We'll need to do a strlen check every place we use the superhud.
	//Superhud - End
	{ &cg_crosshairPulse, "cg_crosshairPulse", "1", CVAR_ARCHIVE },
	{ &cg_oldScoreboard, "cg_oldScoreboard", "0", CVAR_ARCHIVE },
	{ &ch_shadowOffset, "ch_shadowOffset", "1", CVAR_ARCHIVE },
...
===================
*/
static void CG_ForceModelChange( int type ) {
	int		i;
	...
	for (i=0 ; i<MAX_CLIENTS ; i++) {
		...
		if ( type == 0 || (type == 2 && isTeammate(&cgs.clientinfo[i], i) ) || ( type == 1 && isEnemy(&cgs.clientinfo[i], i) ) ) {
			CG_NewClientInfo( i );
		}
	}
}
...
Called to register and re-register custom announcers
=================
*/
static void CG_RegisterAnnouncerSounds( void ) {
	...
	memset(&cgs.media.oneMinuteSound, 0, sizeof(cgs.media.oneMinuteSound));
	Com_sprintf(path, sizeof(path), "sound/%s/1_minute.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/1_minute.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.fiveMinuteSound, 0, sizeof(cgs.media.fiveMinuteSound));
	Com_sprintf(path, sizeof(path), "sound/%s/5_minute.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.fiveMinuteSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/5_minute.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.suddenDeathSound, 0, sizeof(cgs.media.suddenDeathSound));
	Com_sprintf(path, sizeof(path), "sound/%s/sudden_death.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.suddenDeathSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/sudden_death.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.oneFragSound, 0, sizeof(cgs.media.oneFragSound));
	Com_sprintf(path, sizeof(path), "sound/%s/1_frag.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.oneFragSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/1_frag.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.twoFragSound, 0, sizeof(cgs.media.twoFragSound));
	Com_sprintf(path, sizeof(path), "sound/%s/2_frags.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.twoFragSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/2_frags.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.threeFragSound, 0, sizeof(cgs.media.threeFragSound));
		Com_sprintf(path, sizeof(path), "sound/%s/3_frags.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
	cgs.media.threeFragSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/3_frags.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.count3Sound, 0, sizeof(cgs.media.count3Sound));
	Com_sprintf(path, sizeof(path), "sound/%s/three.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.count3Sound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/three.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.count2Sound, 0, sizeof(cgs.media.count2Sound));
	Com_sprintf(path, sizeof(path), "sound/%s/two.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.count2Sound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/two.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.count1Sound, 0, sizeof(cgs.media.count1Sound));
	Com_sprintf(path, sizeof(path), "sound/%s/one.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.count1Sound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/one.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.countFightSound, 0, sizeof(cgs.media.countFightSound));
	Com_sprintf(path, sizeof(path), "sound/%s/fight.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.countFightSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/fight.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.countPrepareSound, 0, sizeof(cgs.media.countPrepareSound));
	Com_sprintf(path, sizeof(path), "sound/%s/prepare.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.countPrepareSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/prepare.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.redLeadsSound, 0, sizeof(cgs.media.redLeadsSound));
	Com_sprintf(path, sizeof(path), "sound/%s/redleads.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.redLeadsSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/redleads.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.blueLeadsSound, 0, sizeof(cgs.media.blueLeadsSound));
	Com_sprintf(path, sizeof(path), "sound/%s/blueleads.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.blueLeadsSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/blueleads.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.teamsTiedSound, 0, sizeof(cgs.media.teamsTiedSound));
	Com_sprintf(path, sizeof(path), "sound/%s/teamstied.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.teamsTiedSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/teamstied.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.impressiveSound, 0, sizeof(cgs.media.impressiveSound));
	Com_sprintf(path, sizeof(path), "sound/%s/impressive.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.impressiveSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/impressive.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.excellentSound, 0, sizeof(cgs.media.excellentSound));
	Com_sprintf(path, sizeof(path), "sound/%s/excellent.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.excellentSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/excellent.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.deniedSound, 0, sizeof(cgs.media.deniedSound));
	Com_sprintf(path, sizeof(path), "sound/%s/denied.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.deniedSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/denied.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.humiliationSound, 0, sizeof(cgs.media.humiliationSound));
	Com_sprintf(path, sizeof(path), "sound/%s/humiliation.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.humiliationSound = trap_S_RegisterSound( path, qtrue );
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/humiliation.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.takenLeadSound, 0, sizeof(cgs.media.takenLeadSound));
	Com_sprintf(path, sizeof(path), "sound/%s/takenlead.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.takenLeadSound = trap_S_RegisterSound( path, qtrue);
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/takenlead.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.tiedLeadSound, 0, sizeof(cgs.media.tiedLeadSound));
	Com_sprintf(path, sizeof(path), "sound/%s/tiedlead.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.tiedLeadSound = trap_S_RegisterSound( path, qtrue);
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/takenlead.wav", qtrue );
	}

	memset(path, 0, sizeof(path));
	memset(&cgs.media.lostLeadSound, 0, sizeof(cgs.media.lostLeadSound));
	Com_sprintf(path, sizeof(path), "sound/%s/lostlead.wav", s_announcer.string);
	if ( CG_FileExists(path) ) {
		cgs.media.lostLeadSound = trap_S_RegisterSound( path, qtrue);
	} else {
		cgs.media.oneMinuteSound = trap_S_RegisterSound( "sound/feedback/lostlead.wav", qtrue );
	}
}
...
//unlagged - client options
		...
		if ( cv->vmCvar == &cl_timeNudge ) {
			CG_Cvar_ClampInt( cv->cvarName, cv->vmCvar, 0, 0 );
		} else if ( cv->vmCvar == &rate ) {
			CG_Cvar_ClampInt( cv->cvarName, cv->vmCvar, cgs.minRate, 99999 );
		} else if ( cv->vmCvar == &snaps ) {
			CG_Cvar_ClampInt( cv->cvarName, cv->vmCvar, sv_fps.integer, 125 );
		} else if ( cv->vmCvar == &cg_railHum && cgs.railHum != -2 && cv->vmCvar->integer != cgs.railHum ) {
			//This print out is really stupid since clampInt also prints out a message..
			//But it's for euro's that don't understand that allowed 0 to 0 means only 0 is allowed..
			CG_Printf("^7Warning: cg_railHum is locked to %i on this server.\n", cgs.railHum);
			CG_Cvar_ClampInt( cv->cvarName, cv->vmCvar, cgs.railHum, cgs.railHum);
		} else if ( cv->vmCvar == &com_maxfps && cv->vmCvar->modificationCount != maxFpsModificationCount ) {
			maxFpsModificationCount = cv->vmCvar->modificationCount;
			if ( !(atoi(Info_ValueForKey( CG_ConfigString(CS_SYSTEMINFO), "sv_cheats" ))) ) {
				CG_Cvar_ClampInt( cv->cvarName, cv->vmCvar, 30, 125 );
			}
		} else if ( cv->vmCvar == &cl_maxpackets ) {
			CG_Cvar_ClampInt( cv->cvarName, cv->vmCvar, 30, 125 );
		} else if ( cv->vmCvar == &cg_adjustTeamColors ) {
			CG_Cvar_ClampInt(cv->cvarName, cv->vmCvar, 0, (strlen(cg_TeamColors.string) > strlen(cg_EnemyColors.string)) ? strlen(cg_TeamColors.string) : strlen(cg_EnemyColors.string) );
		} else if ( cv->vmCvar == &s_announcer) {
			CG_RegisterAnnouncerSounds();
		}

//unlagged - client options
		else if( cv->vmCvar == &s_ambient ) {
			if( s_ambient.integer == 0 && !cgSoundOff ) {
				trap_S_StopBackgroundTrack();
				cgSoundOff = qtrue;
			}
			else if( s_ambient.integer != 0 && cgSoundOff ) {
				cgSoundOff = qfalse;
				CG_StartMusic();
			}
		// clamp aimtype to acceptable values
		} else if ( cv->vmCvar == &cg_aimType ) {
			CG_Cvar_ClampInt( cv->cvarName, cv->vmCvar, 0, 3 );
		}
	...
	// Make sure this doesn't have adverse effects
	...
	//PM Skin color cvars modification checks
	if ( colorsModificationCount != colors.modificationCount ) {
		colorsModificationCount = colors.modificationCount;
		//Do a strlen check, can't be less than 5
		if ( strlen(colors.string) != 5 ) {
			CG_Printf("^3Error: Colors must have 5 values\n^3Resetting to default\n");
			trap_Cvar_Set( "Colors", "17776" );
		}
		CG_ParseColors(COLORS, colors.string, strlen(colors.string) );
	}
	if ( teamColorsModificationCount != cg_TeamColors.modificationCount ) {
		...
		if ( strlen(cg_TeamColors.string) && (strlen(cg_TeamColors.string) < 2 || strlen(cg_TeamColors.string) > 5) ) {
			CG_Printf("^3Error: cg_TeamColors must have a minimum of 2 values and a maximum of 5 values\n^3Resetting to default\n");
			trap_Cvar_Set( "cg_TeamColors", "77777" );
		}
		CG_ParseColors(TEAM_COLORS, cg_TeamColors.string, strlen(cg_TeamColors.string) );
	}
	if ( enemyColorsModificationCount != cg_EnemyColors.modificationCount ) {
		...
		if ( strlen(cg_EnemyColors.string) && (strlen(cg_EnemyColors.string) < 2 || strlen(cg_EnemyColors.string) > 5) ) {
			CG_Printf("^3Error: cg_EnemyColors must have a minimum of 2 values and a maximum of 5 values\n^3Resetting to default\n");
			trap_Cvar_Set( "cg_EnemyColors", "77777" );
		}
		CG_ParseColors(ENEMY_COLORS, cg_EnemyColors.string, strlen(cg_EnemyColors.string) );
	}
	//PM Skin color cvars modification checks <- end

	// if force model changed
	if ( forceModelModificationCount != cg_forceModel.modificationCount ) {
		forceModelModificationCount = cg_forceModel.modificationCount;
		CG_ForceModelChange( 0 );
	}
	//Evo:Model:Begin
	if ( enemyModelModificationCount != cg_enemyModel.modificationCount ) {
		enemyModelModificationCount = cg_enemyModel.modificationCount;
		CG_ForceModelChange( 1 );
	}
	if ( teamModelModificationCount != cg_teamModel.modificationCount ) {
		teamModelModificationCount = cg_teamModel.modificationCount;
		CG_ForceModelChange( 2 );
	}
}
...
static void CG_RegisterSounds( void ) {
	...
	//Com_sprintf(path, sizeof(path), "feedback");
	// voice commands
	...
	if ( cgs.gametype >= GT_TEAM || cg_buildScript.integer ) {
		...
		cgs.media.pickedUpYourTeamSound = trap_S_RegisterSound( "sound/teamplay/flagtk_red.wav", qtrue );
	...
	cgs.media.grappleSound = trap_S_RegisterSound("sound/weapons/melee/fstrun.wav", qfalse);
	...
	// CPM: Hit tones
	cgs.media.hitSound[0] = trap_S_RegisterSound( "sound/feedback/hitlowest.wav", qfalse );
...
static void CG_RegisterGraphics( void ) {
	...
	cgs.media.scoreboardTeamnameRed = trap_R_RegisterShaderNoMip( "menu/tab/teamnamered.tga" );
	cgs.media.scoreboardTeamnameBlue = trap_R_RegisterShaderNoMip( "menu/tab/teamnameblue.tga" );
...
//freeze
	...
	if ( cgs.gametype == GT_CTF || cg_buildScript.integer ) {
		...
		cgs.media.whiteFlagShader[0] = trap_R_RegisterShaderNoMip( "icons/iconf_fla1" );
		cgs.media.whiteFlagShader[1] = trap_R_RegisterShaderNoMip( "icons/iconf_fla2" );
		cgs.media.whiteFlagShader[2] = trap_R_RegisterShaderNoMip( "icons/iconf_fla3" );
		cgs.media.whiteFlagShader[3] = trap_R_RegisterShaderNoMip( "icons/iconf_fla4" );
#ifdef MISSIONPACK
...
void CG_Init( int serverMessageNum, int serverCommandSequence, int clientNum ) {
	...
	memset( cg_weapons, 0, sizeof(cg_weapons) );
	memset( cg_items, 0, sizeof(cg_items) );
	memset( autoFile.date, 0, sizeof(autoFile.date) );
	memset( chat, 0, sizeof(chat) );

	cg.clientNum = clientNum;

	cgs.autoScreenshotTaken = qfalse; //Enable autoScreenshots <-- this will probably happen after alt + enter/tab to cause multiple screenshots
	cgs.autoRecording = qfalse;
	cgs.stopRecording = 0;
	cgs.takeScreenshot = 0;
	...
	cgs.media.charsetCPMA		= trap_R_RegisterShader( "fonts/cpma" );
	cgs.media.charsetThreewave	= trap_R_RegisterShader( "fonts/threewave" );
	cgs.media.charsetOSP		= trap_R_RegisterShader( "fonts/osp" );
	cgs.media.charsetRevolution = trap_R_RegisterShader( "fonts/revolution" );
	cgs.media.whiteShader		= trap_R_RegisterShader( "white" );
	cgs.media.charsetProp		= trap_R_RegisterShaderNoMip( "menu/art/font1_prop.tga" );
	cgs.media.charsetPropGlow	= trap_R_RegisterShaderNoMip( "menu/art/font1_prop_glo.tga" );
	cgs.media.charsetPropB		= trap_R_RegisterShaderNoMip( "menu/art/font2_prop.tga" );
	cgs.media.charsetPropC		= trap_R_RegisterShader( "fonts/idblock" );
	cgs.railHum = -2;//Incase it's not a dedicated server.

	CG_RegisterCvars();

	CG_InitConsoleCommands();

	SET_WEAPON(cg.weaponSelect, WP_MACHINEGUN);

	cgs.flags[TEAM_RED-1] = cgs.flags[TEAM_BLUE-1] = -1; // For compatibily, default to unset for
	cgs.flagStatus = -1;
	// old servers
	cg.naftInterval = 0;
	cg.lastHeldFlag = -16;//-16 isn't an item.. garunteed.
	cg.clockInterval = 0;
	cg.score_time = 0;
	cg.score_weapon = 1;
	cgs.autoRecording = qfalse;
	cg.alternateHandicap = qfalse;
	...
	if ( strlen(ch_file.string) ) {
		//Load the superhud
		trap_SendConsoleCommand("reloadHUD");
	}
}
...
//	vec3_t		start;
	...
	//cg_entities[0].currentState.

	VectorCopy( cg.refdef.vieworg, muzzle );
	AngleVectors( cg.refdefViewAngles, forward, NULL, NULL );
	VectorMA( muzzle, 14, forward, muzzle );

	//FIXME: Get these settings from the server so they're not static
	VectorMA (muzzle, cg.weaponSelect==WP_GAUNTLET?(32):(cg.weaponSelect==WP_LIGHTNING?(LIGHTNING_RANGE):(8192)), forward, end);

	CG_Trace (&trace, muzzle, NULL, NULL, end, cg.snap->ps.clientNum, MASK_SHOT );

	//CG_Printf( "Using Client: %d for server sync\n", trace.entityNum );

	//return 4;

	if ( trace.entityNum == cg.snap->ps.clientNum ) {
		return 0;
	}

	if (trace.entityNum >= 0 && trace.entityNum < MAX_CLIENTS) {
		// add 1 to the client num, so we can still use 0 as a fail value.
		return (trace.entityNum + 1);
	}
