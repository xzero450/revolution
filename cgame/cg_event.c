...
static void CG_Obituary( entityState_t *ent ) {
	...
	if ( ci->sex[0] == 'f' || ci->sex[0] == 'F' ) {
		//CG_Printf("female\n");
		gender = GENDER_FEMALE;
	} else if ( ci->sex[0] == 'm' || ci->sex[0] == 'M' ) {
		//CG_Printf("male\n");
		gender = GENDER_MALE;
	} else if ( ci->sex[0] == 'n' || ci->sex[0] == 'N' ) {
		//CG_Printf("neuter\n");
...
//freeze
	...
	if ( attacker == cg.snap->ps.clientNum ) {
		char	*s;
		//int		divideBy;
		//cg_drawDeathMessage.integer==2?(divideBy = 2):(divideBy = 1);
		if ( hud_count && strlen(ch_file.string) ) {
			//FragMessage
			memset(cg.fragMessageChar, 0, sizeof(cg.fragMessageChar) );
			s = va("You fragged %s", targetName );
			Com_sprintf(cg.fragMessageChar, sizeof(cg.fragMessageChar), "%s", s);
			cg.fragMessageTime = cg.time;
			//RankMessage
			if ( cgs.gametype < GT_TEAM ) {
				memset(cg.rankMessageChar, 0, sizeof(cg.rankMessageChar) );
				s = va("%s place with %i", CG_PlaceString( cg.snap->ps.persistant[PERS_RANK] + 1 ), cg.snap->ps.persistant[PERS_SCORE] );
				Com_sprintf(cg.rankMessageChar, sizeof(cg.rankMessageChar), "%s", s);
				cg.rankMessageTime = cg.time;
			}
        } else if ( cgs.gametype < GT_TEAM && !cg.scoreBoardShowing ) {
		...
		if (!(cg_singlePlayerActive.integer && cg_cameraOrbit.integer)) {
			if ( cg_drawDeathMessage.integer ) {
				CG_CenterPrint( s, SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );
			}
		} 
#else
		if ( cg_drawDeathMessage.integer && !(hud_count && strlen(ch_file.string)) ) {
			CG_CenterPrint( s, SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );
		}
#endif
...
static void CG_ItemPickup( int itemNum ) {
	...
	//CG_Printf("CG_ItemPikup --");
	if ( bg_itemlist[itemNum].giType == IT_TEAM ) {
		cg.itemPickupFlag = bg_itemlist[itemNum].giTag;
	}
	...
	if ( bg_itemlist[itemNum].giType == IT_WEAPON ) {
		...
		if ( cg_autoswitch.integer && bg_itemlist[itemNum].giTag != WP_MACHINEGUN ) {
			...
			SET_WEAPON(cg.weaponSelect, bg_itemlist[itemNum].giTag);
		}
	}
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	case EV_FALL_SHORT:
		...
		if ( clientNum == cg.predictedPlayerState.clientNum && cg_fallKick.integer ) {
...
//unlagged - attack prediction #2
		...
		if ( es->clientNum == cg.predictedPlayerState.clientNum && 
				cgs.delagHitscan && cg_aimType.integer > 0 ) {
...
//unlagged - attack prediction #2
	...
	case EV_GLOBAL_SOUND:	// play from the player's head so it never diminishes
		...
		if ( cgs.gameSounds[ es->eventParm ] ) {
			//CG_Printf("if:SoundIndexNum: %i", es->eventParm );
			/*if ( es->eventParm == 11 ) {
				char	path[MAX_QPATH];
				memset(path, 0, sizeof(path));
				Com_sprintf(path, sizeof(path), "sound/%s/accuracy.wav", s_announcer.string);
				trap_SendConsoleCommand(va("play %s \n", path));
				//trap_S_StartSound ( NULL, cg.snap->ps.clientNum, CHAN_AUTO, path );
			} else {*/
				trap_S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_AUTO, cgs.gameSounds[ es->eventParm ] );
			//}
		} else {
			s = CG_ConfigString( CS_SOUNDS + es->eventParm );
			//CG_Printf("else:SoundIndexNum: %i", es->eventParm );
	...
	case EV_GLOBAL_TEAM_SOUND:	// play from the player's head so it never diminishes
		{
			...
			switch( es->eventParm ) {
				case GTS_PICKED_UP_DROP_TEAM_FLAG: // dropped flag was picked up
						CG_AddBufferedSound( cgs.media.pickedUpYourTeamSound );
					break;
