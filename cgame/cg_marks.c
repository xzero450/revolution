...
markPoly_t	cg_markPolys[MAX_MARK_POLYS];//Added *4
...
void CG_AddMarks( void ) {
	...
	for ( ; mp != &cg_activeMarkPolys ; mp = next ) {
		...
		if ( ( cg.time > mp->time + MARK_TOTAL_TIME && cg_addMarks.integer == 1 ) ||
			( cg.time > mp->time + (cg_addMarks.integer * 1000) && cg_addMarks.integer > 1 && cg_addMarks.integer < 1000 ) ||
			( cg.time > mp->time + ((cg_addMarks.integer - 1000) * 1000) && cg_addMarks.integer >= 1000) ) {
			CG_FreeMarkPoly( mp );
			continue;
		}
		...
		if ( cg_addMarks.integer == 1 || cg_addMarks.integer > 1000) {
			//Do normal fade timing
			t = mp->time + MARK_TOTAL_TIME - cg.time;
		} else if ( cg_addMarks.integer > 1 && cg_addMarks.integer < 1000 ) {
			//Time corrected fading
			t = (mp->time + (cg_addMarks.integer * 1000)) + MARK_TOTAL_TIME - cg.time;
		}
		if ( t < MARK_FADE_TIME ) {
