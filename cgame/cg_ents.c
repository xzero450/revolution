...
static void CG_Item( centity_t *cent ) {
	...
	//itemFx & 2 could've been done here I guess.. 
...
static void CG_Grapple( centity_t *cent ) {
	...
	if (weap_enableHook.integer == 1) {
		trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.grappleSound);
	}
...
//unlagged - smooth clients #2
	...
	//if ( !g_unlaggedSmoothClients.integer && !cg_smoothClients.integer ) {
...
//unlagged - early transitioning
	...
	for ( num = 0 ; num < cg.snap->numEntities ; num++ ) {
		...
		// only accept events from myself for now.
...
/*
int AllowEntityInteraction(centity_t *ent2)
{
	int clientNum1 = 0;
	int clientNum2 = 0;
	//centity_t *ent1;
	...
	clientNum1 = cg.snap->ps.clientNum;
	clientNum2 = ent2->currentState.clientNum;
	...
	{
		clientNum2 = ent2->currentState.otherEntityNum;
	}

	if (clientNum2 < 0 || clientNum2 >= MAX_CLIENTS)
	{
		clientNum2 = ent2->currentState.otherEntityNum2;
	}
...
*/
/**/
	// need to special case items for specing warmup players
/*	if (ent2->currentState.eType == ET_ITEM)
	{
		if (ent2->currentState.generic1 & (1<<cgs.clientinfo[clientNum1].team))
		{
			return 0;
		}
		else
...
/*
	...
	if (cgs.clientinfo[clientNum2].team == TEAM_SPECTATOR)
	{
			return 0;
	}
*/
/*	if (cgs.clientinfo[clientNum2].team == TEAM_WARMUP)
	{
		if (cgs.clientinfo[clientNum1].team != TEAM_WARMUP)
			return 0;
	}
...
{
	return AllowEntityInteraction ( &cg_entities[ entityNum ] );
}
*/
